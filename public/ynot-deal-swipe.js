(()=>{
  ...
  function openCard(target){if(!(target instanceof HTMLElement))return;target.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window}))}
  function move(direction){const list=cards();if(list.length<2)return;const current=selectedTitle();let index=list.findIndex(card=>titleOf(card)===current);if(index<0){const active=list.findIndex(card=>card.classList.contains('active'));index=active>=0?active:0}const target=list[(index+direction+list.length)%list.length];openCard(target);requestAnimationFrame(()=>{if(selectedTitle()===current&&target instanceof HTMLElement)target.click()})}
  ...
  function syncControls(){queued=false;ensureControls();const show=Boolean(selected()&&window.innerWidth>=900&&cards().length>1);prev?.classList.toggle('visible',show);next?.classList.toggle('visible',show);if(prev)prev.disabled=!show;if(next)next.disabled=!show}
