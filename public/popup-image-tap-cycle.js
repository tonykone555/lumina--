(()=>{
  ...
  async function cycle(card){resetForProduct(card);const now=Date.now();if((lastCycle.get(card)||0)>now-350)return;lastCycle.set(card,now);lockWorld();const media=await loadMedia(card);if(media.length<2)return;const current=card.dataset.ynotFullMediaUrl||currentImage(card);let index=media.findIndex(url=>url===current);if(index<0){const saved=Number(card.dataset.ynotTapMediaIndex);index=Number.isFinite(saved)?saved:0}const next=(index+1)%media.length;card.dataset.ynotTapMediaIndex=String(next);preload(media[(next+1)%media.length]);showMedia(card,media[next])}
  ...
  /* On mobile, only remember the start point here. The click event owns the actual
     image advance so the same tap can never advance twice or fall through to the viewer. */
  document.addEventListener('pointerdown',e=>{
    const hit=mainDealImage(e.target);
    if(mobile()&&hit){lockWorld();starts.set(e.pointerId,{x:e.clientX,y:e.clientY,card:hit.card,mobileDeal:true});return}
  ...
  document.addEventListener('pointerup',e=>{
    ...
    if(s.mobileDeal){lockWorld();return}
  ...
  document.addEventListener('click',event=>{
    ...
    if(mobile()&&hit){event.preventDefault();event.stopImmediatePropagation();lockWorld();void cycle(hit.card);return}
  ...
  if(!window.PointerEvent){
    document.addEventListener('touchstart',e=>{const hit=mainDealImage(e.target);if(!hit)return;const touch=e.changedTouches?.[0];if(!touch)return;lockWorld();hit.card.__ynotTouchStart={x:touch.clientX,y:touch.clientY,time:Date.now()}},{passive:true,capture:true});
