(()=>{
  ...
  document.addEventListener('pointerup',e=>{const s=starts.get(e.pointerId);if(!s)return;starts.delete(e.pointerId);const dx=e.clientX-s.x,dy=e.clientY-s.y;if(s.mobileDeal){lockWorld();if(Math.hypot(dx,dy)<=16){e.preventDefault();e.stopImmediatePropagation();s.card.__ynotLastPointerCycle=Date.now();void cycle(s.card)}return}if(Math.hypot(dx,dy)<=14)void cycle(s.card)},true);
  document.addEventListener('pointercancel',e=>starts.delete(e.pointerId),true);
  document.addEventListener('click',event=>{const hit=mainDealImage(event.target);if(mobile()&&hit){event.preventDefault();event.stopImmediatePropagation();lockWorld();if(Date.now()-(hit.card.__ynotLastPointerCycle||0)>420)void cycle(hit.card);return}const target=event.target;if(!(target instanceof Element))return;const card=target.closest('.lv4-detail,.ynot-selected');if(!card)return;const key=resetForProduct(card);const thumb=target.closest('.ynot-loaded-gallery button,.ynot-rich-gallery button,.lv4-gallery button,.ynot-deal-thumb-gallery button');if(thumb){const url=thumbUrl(thumb);if(url){event.preventDefault();event.stopPropagation();showMedia(card,url,key)}return}if(target instanceof HTMLVideoElement&&target.classList.contains('ynot-popup-video')){event.preventDefault();event.stopPropagation();void cycle(card);return}if(target===card.querySelector(':scope > img')){event.preventDefault();event.stopPropagation();void cycle(card);return}if(card.classList.contains('ynot-selected')){if(target.closest('button,a,input,select,textarea,video,.ynot-deal-thumb-gallery,.ynot-selected-copy,.ynot-variants'))return;event.preventDefault();event.stopPropagation();void cycle(card)}},true);
  ...
  let lastOpenKey='';
  const observer=new MutationObserver(()=>{document.querySelectorAll('.lv4-detail,.ynot-selected').forEach(card=>{const key=resetForProduct(card);if(card.matches('.ynot-drawer.open .ynot-selected')&&key&&key!==lastOpenKey){lastOpenKey=key;delete card.dataset.ynotFullMediaUrl;delete card.dataset.ynotTapMediaIndex}const main=card.querySelector(':scope > img');if(main){main.decoding='async';main.style.cursor='pointer';main.style.touchAction='manipulation'}void loadMedia(card)})});
