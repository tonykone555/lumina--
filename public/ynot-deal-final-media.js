(()=>{
  ...
  async function rebuildGallery(shell){
    ...
    existing?.remove();
    const gallery=document.createElement('div');gallery.className='ynot-deal-thumb-gallery ynot-deal-thumb-gallery-final';gallery.dataset.signature=`${document.documentElement.classList.contains('ynot-deal-etsy-only')?'etsy':'all'}:${norm(title)}`;gallery.dataset.finalSignature=signature;
    ...
    shell.dataset.ynotFinalMedia=JSON.stringify(media);
  }
