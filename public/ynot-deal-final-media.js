(()=>{
  ...
  function removeSliderChrome(shell){
    shell.querySelectorAll('.ynot-full-slider,.ynot-deal-full-slider,.ynot-full-slider-prev,.ynot-full-slider-next,.ynot-full-slider-count,.ynot-deal-thumb-more').forEach(node=>node.remove());
  }
  ...
  async function rebuildGallery(shell){
    ...
    const signature=`boxes-only-v1:${norm(title)}`;const existing=shell.querySelector('.ynot-deal-thumb-gallery');if(existing?.dataset.finalSignature===signature)return;
    ...
    existing?.remove();removeSliderChrome(shell);
    ...
    media.forEach((url,index)=>{
      const b=document.createElement('button');b.type='button';b.dataset.mediaUrl=url;b.className='ynot-deal-thumb';b.setAttribute('aria-label',`Show product media ${index+1}`);b.appendChild(makeThumb(url,title,index));if(index===0)b.classList.add('active');
  ...
  let queued=false;function sync(){queued=false;document.querySelectorAll('.ynot-drawer.open .ynot-selected').forEach(shell=>{removeSliderChrome(shell);syncHeart(shell);void rebuildGallery(shell)})}
