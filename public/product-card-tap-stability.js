(()=>{
  ...
  function gestureLocked(){return Number(window.__ynotProductGestureLockUntil||0)>Date.now()}
  document.addEventListener('pointerdown',event=>{
    if(synthetic||event.pointerType==='mouse'||popupOpen()||gestureLocked()){start=null;return}
  ...
  document.addEventListener('pointerup',event=>{
    ...
    if(popupOpen()||gestureLocked())return;
    const card=cardFrom(event.target);
    ...
    setTimeout(()=>{
      if(popupOpen()||gestureLocked()||!document.contains(current.card))return;
      synthetic=true;
