...
export default async function ShoppingCategoryPage({params}:Props){
  ...
  const url=`${BASE}/shop/${category.slug}`;
  const related=SEO_CATEGORIES.filter(item=>item.slug!==category.slug);
  const jsonLd={
  ...
  return <main style={{minHeight:"100dvh",background:"#080909",color:"#f6f6f2",fontFamily:"ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif",padding:"clamp(28px,6vw,76px)"}}>
    ...
    <section style={{maxWidth:860,margin:"0 auto"}}>
      ...
      <p style={{fontSize:"clamp(18px,2.4vw,27px)",lineHeight:1.35,letterSpacing:"-.025em",opacity:.78,maxWidth:760,margin:"0 0 28px"}}>{category.intro}</p>
      <p style={{fontSize:16,lineHeight:1.7,opacity:.67,maxWidth:760,margin:"0 0 46px"}}>{category.guide}</p>
      ...
      <nav aria-label="Related shopping categories" style={{borderTop:"1px solid rgba(255,255,255,.13)",marginTop:56,paddingTop:28}}>
        <p style={{fontSize:12,textTransform:"uppercase",letterSpacing:".16em",opacity:.45,margin:"0 0 16px"}}>Explore more shopping worlds</p>
        <div style={{display:"flex",flexWrap:"wrap",gap:10}}>{related.map(item=><Link key={item.slug} href={`/shop/${item.slug}`} style={{color:"inherit",textDecoration:"none",border:"1px solid rgba(255,255,255,.14)",borderRadius:999,padding:"10px 14px",background:"rgba(255,255,255,.04)",fontSize:14}}>{item.name}</Link>)}</div>
      </nav>
    </section>
  </main>;
}
