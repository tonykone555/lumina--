...
export default function CreatorDashboard(){
 ...
 async function promote(p:Product){
  ...
  try{const r=await authedFetch("/api/earn/products",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(p)}),j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||"Could not add product");await load();setTab("links");setNotice("Product added. Your personal link is ready.")}catch(e){setNotice(e instanceof Error?e.message:"Could not add product")}finally{setWorking("")}
 }
