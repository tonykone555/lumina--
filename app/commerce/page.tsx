...
export default function CommercePage(){
  ...
  return <main style={{minHeight:"100vh",background:"#f1ede7",color:"#27231f",fontFamily:"Inter,system-ui,sans-serif",padding:"28px"}}>
    <div style={{maxWidth:1180,margin:"0 auto"}}>
      <p style={{fontSize:11,letterSpacing:".14em",textTransform:"uppercase",opacity:.55}}>Lumina @ / Commerce Control</p>
      ...
      <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12,marginTop:24}}>
        <Metric label="Lumina price" value={quote?eur(quote.luminaPrice):"…"} sub="Dynamic risk-adjusted pricing"/>
      ...
      <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:14,marginTop:14}}>
        <Card title="Dynamic margin engine" text="Category-specific return reserves, cancellation reserve, payment fees, regional risk and reliability all feed the final Lumina price." tags={["Landed cost","Risk floor","Price lock","Margin floor"]}/>
        ...
        <Card title="Lumina Wallet + Credit" text={`Current demo wallet: ${eur(wallet)}. Credits can absorb price-protection rewards, refunds, referral bonuses and loyalty value.`} tags={["Price protection","Referral credit","Refund credit"]} action={()=>setWallet(v=>Math.round((v+10)*100)/100)} actionLabel="Add €10 demo credit"/>
        <Card title="Basket optimizer" text="Each basket line can carry multiple source options. Lumina chooses the strongest risk-adjusted route and reports expected total contribution before checkout." tags={["Shared shipping","Source consolidation","Split orders"]}/>
        ...
        <Card title="Lumina+" text="Consumer membership can later fund premium image-reference search, saved worlds, price alerts, visualization credits and delivery benefits." tags={["Recurring revenue","Saved worlds","Price alerts"]}/>
        <Card title="Lumina Fulfilled" text="A future fulfillment mode is already represented in routing logic, allowing high-volume products to graduate from retail sourcing to direct warehouse fulfillment." tags={["Higher margin","Direct stock","Fulfillment network"]}/>
      ...
      <section style={{marginTop:14,padding:20,borderRadius:28,background:"rgba(255,255,255,.45)",border:"1px solid rgba(255,255,255,.6)"}}>
        ...
        <h2 style={{fontFamily:"Georgia,serif",fontStyle:"italic",fontWeight:500,fontSize:30,margin:"6px 0 8px"}}>Discovery → verified price → Lumina checkout → smart fulfillment → direct merchant leverage.</h2>
        <p style={{opacity:.63,maxWidth:900,lineHeight:1.55,margin:0}}>The logic is now separated from the discovery UI so Lumina can keep its invisible bubble experience while the commerce engine makes the commercial decision underneath.</p>
      </section>
    </div>
  </main>
}
