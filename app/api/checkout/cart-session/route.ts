...
export async function POST(req:NextRequest){
 try{
  ...
  for(const raw of items){
   ...
   if(String(shipping.currency||itemCurrency).toUpperCase()!==currency)throw new Error("SHIPPING_CURRENCY_MISMATCH");
   prepared.push({product,quantity,customerPrice,shipping,variantLabel:raw.variantLabel});
  }
