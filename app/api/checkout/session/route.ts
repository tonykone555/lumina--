...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const customerPrice=Number(signed.price),shipping=rawShipping,subtotalCents=Math.round(customerPrice*quantity*100),jar=await cookies(),buyerId=jar.get("ynot-circle-id")?.value||"",creatorId=jar.get("ynot-creator-id")?.value||"",creatorRef=jar.get("ynot-creator-ref")?.value||"",creatorProductId=jar.get("ynot-creator-product")?.value||"";
