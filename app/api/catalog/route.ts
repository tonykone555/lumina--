...
function isFurniture(p:Product){return inferCategory(p)==="home"&&/\\b(sofa|sectional|couch|armchair|recliner|ottoman|chair|bed|mattress|dining table|coffee table|side table|desk|cabinet|dresser|wardrobe|bookshelf|shelving|sideboard|console|bench|stool)\\b/i.test(`${p.title} ${(p.tags||[]).join(" ")}`)}
function validCustomerPrice(p:Product){const price=Number(p.price);if(!Number.isFinite(price)||price<=0)return false;/* Never guess/correct an uncertain furniture price. Three-digit minimum: malformed/sub-€100 furniture is withheld from every catalogue surface. */return !isFurniture(p)||price>=100}
function customerSafe(products:Product[]){return products.filter(validCustomerPrice)}
