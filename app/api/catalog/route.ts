...
function withYnotPrice(p:Product):Product{let supplierPrice=Number(p.supplierPrice??p.price);if(!Number.isFinite(supplierPrice)||supplierPrice<=0)return p;const category=inferCategory(p);/* Some Shopify Global Catalog furniture merchants expose major-looking values 100x too small (e.g. 1.71 for 171). Never let those malformed minor-unit values become customer-facing YNOT prices. */const furnitureLike=/\b(sofa|sectional|couch|armchair|chair|ottoman|bed|mattress|table|desk|cabinet|dresser|wardrobe|bookshelf|shelving|sideboard|console)\b/i.test(`${p.title} ${(p.tags||[]).join(" ")}`);if(category==="home"&&furnitureLike&&supplierPrice>0&&supplierPrice<25)supplierPrice*=100;const retailPrice=displayPrice(p,supplierPrice);return{...p,supplierPrice,retailPrice,price:retailPrice,pricingMode:"ynot-retail"}}
