...
export async function GET(req:NextRequest){
 try{
  ...
  const limit=Math.max(20,Math.min(500,Number(req.nextUrl.searchParams.get("limit"))||250));
  const offset=Math.max(0,Number(req.nextUrl.searchParams.get("offset"))||0);
  const rows=await adminDb(`ynot_growth_opportunities?kind=eq.intent&select=id,external_key,kind,platform,handle,display_name,profile_url,source_post_url,niche,country,followers,engagement,intent_strength,creator_fit,summary,reason,source_quote,budget_min,budget_max,budget_currency,intent_tags,constraints,personalization_context,matched_product_ids,matched_products,draft_message,channel,status,owner,outreach_approved,contacted_at,next_action,created_at,updated_at&order=updated_at.desc&limit=${limit}&offset=${offset}`);
  return NextResponse.json({ok:true,opportunities:Array.isArray(rows)?rows:[],limit,offset});
 }catch(e){const m=e instanceof Error?e.message:"INTENT_OUTREACH_LOAD_FAILED";return NextResponse.json({error:m},{status:adminErrorStatus(e)})}
}
