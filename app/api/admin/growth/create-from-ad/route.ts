...
async function saveCreative(input:{ad:any;product:any;analysis:any;direction:any;variants:any[];scores:any[];folderId?:string}){
 ...
 const sorted=[...scored].sort((a,b)=>Number(b.score?.overall||0)-Number(a.score?.overall||0));
 const best=sorted[0];
 const payload={
  source:"growth-ad-intelligence",source_ad:input.ad,ad_analysis:input.analysis?.analysis,adaptation:input.analysis?.adaptation,
  creative_direction:input.direction,generated_variants:scored,provider:"modal-sdxl-img2img",folder_id:input.folderId||null,
  product:{id:productId(input.product),title:input.product?.title,image:productImage(input.product),price:input.product?.price,currency:input.product?.currency,brand:input.product?.brand}
 };
...
export async function POST(req:NextRequest){
 try{
  ...
  if(action==="analyse"){
   if(!ad?.id&&!ad?.headline&&!ad?.body&&!ad?.imageUrl)return NextResponse.json({error:"SOURCE_AD_REQUIRED"},{status:400});
   const analysis=await analyseAdForProduct({ad,product,contextAds:Array.isArray(body?.contextAds)?body.contextAds:[]});
   return NextResponse.json({ok:true,stage:"analysed",analysis,modalConfigured:modalGrowthImageConfigured()});
  }
  if(action==="generate"){
