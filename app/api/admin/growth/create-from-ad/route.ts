...
async function saveCreative(input:{ad:any;product:any;analysis:any;direction:any;variants:any[];scores:any[];folderId?:string}){
 ...
 const payload={source:"growth-ad-intelligence",source_ad:input.ad,ad_analysis:input.analysis?.analysis,adaptation:input.analysis?.adaptation,creative_direction:input.direction,generated_variants:scored,provider:"modal-sdxl-img2img",folder_id:input.folderId||null,product:{id:productId(input.product),title:input.product?.title,image:productImage(input.product),price:input.product?.price,currency:input.product?.currency,brand:input.product?.brand}};
 ...
 if(input.folderId){try{const researchRows=scored.map((v:any)=>({folder_id:input.folderId,kind:"generated-ad",source:"YNOT · Gemini + Modal",title:`${input.direction?.name||"Creative direction"} · Variant ${v.variantIndex}`,summary:`Original YNOT creative generated from market pattern analysis for ${input.product?.title||"selected product"}.`,image_url:v.url,score:Number(v.score?.overall||0)||null,payload:{creative_id:creative?.id||null,source_ad_id:input.ad?.id||null,product_id:productId(input.product),direction:input.direction,score:v.score,seed:v.seed,provider:"modal-sdxl-img2img"}}));await adminDb("ynot_growth_research_items",{method:"POST",headers:{Prefer:"return=minimal"},body:JSON.stringify(researchRows)})}catch(e){console.error("create_from_ad_research_save",e)}}
