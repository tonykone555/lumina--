...
async function counts(){
 const [niches,creatives,jobs,campaigns,strategies]=await Promise.all([
  adminDb("ynot_ad_niches?select=id,name,category,subcategory,attributes,query,status,created_at&order=created_at.desc&limit=80"),
  adminDb("ynot_ad_creatives?select=id,status,quality_score,template_key,aspect_ratio,hook,headline,cta,payload,source_product_ids,created_at,updated_at&order=created_at.desc&limit=80"),
  adminDb("ynot_ad_jobs?select=id,status,job_type,creative_id,provider,output,created_at&order=created_at.desc&limit=80"),
  adminDb("ynot_ad_campaigns?select=id,status,platform,daily_budget,currency,name,objective,created_at&order=created_at.desc&limit=80"),
  adminDb("ynot_ad_strategies?select=id,name,objective,country,platform,daily_budget,currency,niche_queries,creative_mix,test_plan,status,created_at&order=created_at.desc&limit=40")
 ]);
 ...
 return {
  ...
  strategies:{total:count(strategies),draft:count(strategies,"draft"),active:count(strategies,"active")},
  recentNiches:niches.slice(0,20),recentCreatives:creatives.slice(0,20),recentJobs:jobs.slice(0,20),recentCampaigns:campaigns.slice(0,20),recentStrategies:strategies.slice(0,12)
 };
}
...
export async function POST(req:NextRequest){
 try{
  ...
  if(action==="create_niche"){
   ...
   const rows=await adminDb("ynot_ad_niches",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({name,category:String(body?.category||"").slice(0,80)||null,subcategory:String(body?.subcategory||"").slice(0,80)||null,attributes:Array.isArray(body?.attributes)?body.attributes.slice(0,12):[],query:String(body?.query||name).slice(0,500),status:String(body?.status||"draft").slice(0,20),created_by:admin.profile.id})});
   return NextResponse.json({ok:true,niche:rows?.[0]});
  }

  if(action==="create_strategy"){
   const name=String(body?.name||"YNOT test plan").trim().slice(0,140);
   const rows=await adminDb("ynot_ad_strategies",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({created_by:admin.profile.id,name,objective:String(body?.objective||"sales").slice(0,80),country:String(body?.country||"France").slice(0,80),platform:String(body?.platform||"Meta + Instagram").slice(0,100),daily_budget:Number(body?.dailyBudget||10),currency:String(body?.currency||"EUR").slice(0,10),niche_queries:Array.isArray(body?.nicheQueries)?body.nicheQueries.slice(0,20):[],creative_mix:body?.creativeMix||{},test_plan:body?.testPlan||{},status:"draft"})});
  ...
  if(action==="create_creative"){
   ...
   const payload={products:Array.isArray(body?.products)?body.products.slice(0,12):[],niche:body?.niche||null,format:body?.aspectRatio||"9:16",createdFrom:"ynot-ad-factory-v2"};
  ...
  if(action==="update_creative_status"){
   const creativeId=String(body?.creativeId||"");const status=String(body?.status||"");
   if(!creativeId||!["draft","approved","rejected"].includes(status))return NextResponse.json({ok:false,error:"INVALID_CREATIVE_UPDATE"},{status:400});
   const rows=await adminDb(`ynot_ad_creatives?id=eq.${encodeURIComponent(creativeId)}`,{method:"PATCH",headers:{Prefer:"return=representation"},body:JSON.stringify({status,updated_at:new Date().toISOString()})});
   return NextResponse.json({ok:true,creative:rows?.[0]});
  }

  if(action==="regenerate_creative"){
   const creativeId=String(body?.creativeId||"");if(!creativeId)return NextResponse.json({ok:false,error:"CREATIVE_REQUIRED"},{status:400});
   const source=(await adminDb(`ynot_ad_creatives?id=eq.${encodeURIComponent(creativeId)}&select=*&limit=1`))?.[0];if(!source)return NextResponse.json({ok:false,error:"CREATIVE_NOT_FOUND"},{status:404});
   const rows=await adminDb("ynot_ad_creatives",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({niche_id:source.niche_id,created_by:admin.profile.id,source_product_ids:source.source_product_ids,template_key:source.template_key,aspect_ratio:source.aspect_ratio,hook:source.hook,primary_text:source.primary_text,headline:source.headline,cta:source.cta,payload:{...(source.payload||{}),regeneratedFrom:creativeId,variantSeed:Date.now()},status:"draft"})});
   return NextResponse.json({ok:true,creative:rows?.[0]});
  }
  ...
  if(action==="create_campaign_draft"){
   const creativeId=String(body?.creativeId||"");if(!creativeId)return NextResponse.json({ok:false,error:"CREATIVE_REQUIRED"},{status:400});
   const rows=await adminDb("ynot_ad_campaigns",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({name:String(body?.name||"YNOT test campaign").slice(0,140),platform:String(body?.platform||"meta").slice(0,40),objective:String(body?.objective||"sales").slice(0,80),daily_budget:Number(body?.dailyBudget||10),currency:String(body?.currency||"EUR").slice(0,10),status:"draft",created_by:admin.profile.id,settings:{creativeId}})});
