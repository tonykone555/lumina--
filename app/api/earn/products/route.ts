...
export async function POST(req:NextRequest){
 try{
  ...
  const price=Number(b.price),defaultRate=Number(c.commission_rate||.05),creatorPayout=Number(b.creator_payout),hasCreatorPayout=Number.isFinite(creatorPayout)&&creatorPayout>0&&Number.isFinite(price)&&price>0,rate=hasCreatorPayout?Math.min(.9,creatorPayout/price):defaultRate,commissionCents=Number.isFinite(price)&&price>0?Math.round((hasCreatorPayout?creatorPayout:price*rate)*100):null;
  const sourcing=b.sourcing&&typeof b.sourcing==="object"?b.sourcing:null;
  const payload={creator_id:c.id,product_id:id,title,brand:String(b.brand||"YNOT").slice(0,160),image_url:image,product_url:url,price:Number.isFinite(price)?price:null,currency:String(b.currency||"EUR").toUpperCase().slice(0,5),category:String(b.category||"").slice(0,100)||null,commission_rate:rate,commission_cents:commissionCents,status:"active",metadata:{description:String(b.description||"").slice(0,1000),creator_payout:hasCreatorPayout?creatorPayout:null,sourcing},updated_at:new Date().toISOString()};
