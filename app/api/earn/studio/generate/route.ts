...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const productId=String(product.product_id||product.id||"").slice(0,500),title=String(product.title||"").slice(0,240),image=String(product.image_url||product.image||"").slice(0,1600),description=String(product.description||"").slice(0,2400),category=String(product.category||"").slice(0,240),brand=String(product.brand||"").slice(0,240),mode=String(b.mode||"video-avatar"),angle=String(b.angle||"testimonial"),qualityTier=(String(b.quality_tier||"standard")==="premium"?"premium":"standard") as StudioQualityTier,standardProvider=(String(b.standard_provider||"modal")==="muapi"?"muapi":"modal") as "modal"|"muapi",imageSource=["ynot_standard","ynot_premium","google_flow"].includes(String(b.image_source))?String(b.image_source):"ynot_standard",flowImagePath=String(b.flow_image_path||"").slice(0,1200);
  ...
  if(reuseBasePath){
    ...
    baseImageModel="reused-existing-base";
    baseImagePrompt=String(b.reuse_base_image_prompt||"").slice(0,12000);
  }else if(imageSource==="google_flow"){
  ...
  const draft=(await rest("ynot_creator_studio_jobs",{method:"POST",body:JSON.stringify([{creator_id:c.id,product_id:productId,product_title:title,product_image_url:image,source_path:sourcePath||null,source_video_url:sourceVideoUrl||null,avatar_image_url:avatarImageUrl||null,mode,angle,status:"generating",metadata:{quality_tier:qualityTier,standard_provider:standardProvider,image_source:imageSource,base_image_path:basePath,base_image_model:baseImageModel,base_image_prompt:baseImagePrompt,base_image_reused:Boolean(reuseBasePath),background_image_url:backgroundImageUrl||null,avatar_source:avatarPath?"upload":avatarImageUrl?"remote":"none"}}])}))?.[0];
  try{
    const result=await renderStudioVideo({mode,qualityTier,standardProvider,productTitle:title,productDescription:description||undefined,productCategory:category||undefined,productBrand:brand||undefined,productImageUrl:image,baseImageUrl,sourceVideoUrl:sourceVideoUrl||undefined,avatarImageUrl:avatarImageUrl||undefined,backgroundImageUrl:backgroundImageUrl||undefined,angle});
    const rows=await rest(`ynot_creator_studio_jobs?id=eq.${draft.id}`,{method:"PATCH",body:JSON.stringify({status:"completed",provider_model:result.model,provider_request_id:result.requestId||null,prompt:(result as any).prompt||baseImagePrompt||null,result_url:result.url,thumbnail_url:result.thumbnail,metadata:{quality_tier:qualityTier,standard_provider:standardProvider,image_source:imageSource,base_image_path:basePath,base_image_model:baseImageModel,base_image_reused:Boolean(reuseBasePath),video_provider:(result as any).provider||"higgsfield",routing_tier:result.routingTier,routing_reason:result.routingReason},completed_at:new Date().toISOString(),updated_at:new Date().toISOString()})});
 ...
 }catch(e){const m=e instanceof Error?e.message:"STUDIO_GENERATION_FAILED";return NextResponse.json({error:m,...(typeof basePath!=="undefined"&&basePath?{base_image_path:basePath,base_image_prompt:baseImagePrompt}: {})},{status:/SIGN_IN|SESSION/.test(m)?401:/NOT_CONFIGURED/.test(m)?503:400})}
}
