...
export async function POST(req:NextRequest){
 try{
  ...
  const baseImage=await composeStudioBaseImage({qualityTier,product:{title,description:description||undefined,category:category||undefined,brand:brand||undefined},productImageUrl:image,avatarImageUrl:avatarImageUrl||undefined,backgroundImageUrl:backgroundImageUrl||undefined,mode,angle});
    const ext=baseImage.mimeType.includes("jpeg")?"jpg":baseImage.mimeType.includes("webp")?"webp":"png";
    const basePath=`generated/${c.id}/${Date.now()}-${qualityTier}-base.${ext}`;
    const baseImageUrl=await storeGeneratedImage(basePath,baseImage.mimeType,baseImage.data);
    const draft=(await rest("ynot_creator_studio_jobs",{method:"POST",body:JSON.stringify([{creator_id:c.id,product_id:productId,product_title:title,product_image_url:image,source_path:sourcePath||null,source_video_url:sourceVideoUrl||null,avatar_image_url:avatarImageUrl||null,mode,angle,status:"generating",metadata:{quality_tier:qualityTier,base_image_path:basePath,base_image_model:baseImage.model,base_image_prompt:baseImage.prompt,background_image_url:backgroundImageUrl||null,avatar_source:avatarPath?"upload":avatarImageUrl?"remote":"none"}}])}))?.[0];
  try{
