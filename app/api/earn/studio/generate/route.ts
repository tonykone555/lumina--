...
export async function POST(req:NextRequest){
 try{
  ...
  const draft=(await rest("ynot_creator_studio_jobs",{method:"POST",body:JSON.stringify([{creator_id:c.id,product_id:productId,product_title:title,product_image_url:image,source_path:sourcePath||null,source_video_url:sourceVideoUrl||null,avatar_image_url:avatarImageUrl||null,mode,angle,status:"generating",metadata:{background_image_url:backgroundImageUrl||null,avatar_source:avatarPath?"upload":avatarImageUrl?"remote":"none"}}])}))?.[0];
  try{
    const result=await renderStudioVideo({mode,productTitle:title,productDescription:description||undefined,productCategory:category||undefined,productBrand:brand||undefined,productImageUrl:image,sourceVideoUrl:sourceVideoUrl||undefined,avatarImageUrl:avatarImageUrl||undefined,backgroundImageUrl:backgroundImageUrl||undefined,angle});
