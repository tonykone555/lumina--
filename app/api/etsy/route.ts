...
import {etsyApiKey,getEtsyAccessToken,setEtsyTokenCookies,type EtsyTokenPayload} from "@/lib/etsy/auth";
...
export async function GET(request:NextRequest){
 ...
 const tokenState=await getEtsyAccessToken(request);
 ...
 if(mode==="status")return withTokenCookies(NextResponse.json({connected:Boolean(accessToken),oauth:Boolean(accessToken),source:"etsy"}),tokenState.refreshed);
 if(mode==="reviews"){
  ...
  return withTokenCookies(NextResponse.json(await reviewsFor(listingId)),tokenState.refreshed);
 }
 ...
 try{
  ...
  if(!ids.length)return withTokenCookies(NextResponse.json({products:[],source:"etsy",page,total:Number(active.count||0),oauthConnected:Boolean(accessToken)}),tokenState.refreshed);
  ...
  return withTokenCookies(NextResponse.json({products:enriched.filter(p=>p.image),source:"etsy",page,total:Number(active.count||0),nextPage:page+1,oauthConnected:Boolean(accessToken)}),tokenState.refreshed);
 }catch(error){return withTokenCookies(NextResponse.json({error:error instanceof Error?error.message:"ETSY_UNAVAILABLE"},{status:502}),tokenState.refreshed)}
}
