...
export async function GET(request:NextRequest){
 ...
 const reviewsFor=async(listingId:number)=>{try{const data=await etsy(`/listings/${listingId}/reviews?limit=5&offset=0`) as {count?:number;results?:EtsyReview[]};const reviews=Array.isArray(data.results)?data.results.slice(0,5):[];const valid=reviews.filter(r=>Number(r.rating)>0);const average=valid.length?valid.reduce((sum,r)=>sum+Number(r.rating||0),0)/valid.length:null;return{rating:average?Math.round(average*10)/10:null,reviewCount:Number(data.count||reviews.length),reviews:reviews.map(r=>({rating:Number(r.rating||0),text:String(r.review||"").trim(),createdAt:Number(r.created_timestamp||r.create_timestamp||0),image:r.image_url_fullxfull||null}))}}catch{return{rating:null,reviewCount:0,reviews:[]}}};
