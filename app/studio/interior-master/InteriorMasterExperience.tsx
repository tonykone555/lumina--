...
const CHAPTERS=[
  {label:"01 / ARRIVAL",title:"Enter the atmosphere"},
  {label:"02 / MATERIAL",title:"Move through texture"},
  {label:"03 / DETAIL",title:"Notice what stays"},
];
  ...
  return <main className={styles.interiorMaster}>
    <header className={styles.nav}>
      <button className={styles.wordmark} onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} aria-label="Back to top">
        <span data-personalize="company-name">{STUDIO.name}</span><small>{STUDIO.descriptor}</small>
      </button>
      ...
      <nav className={`${styles.navLinks} ${menuOpen?styles.navOpen:""}`}>
        <button onClick={()=>scrollTo("tour")}>Experience</button><button onClick={()=>scrollTo("projects")}>Projects</button><button onClick={()=>scrollTo("studio")}>Studio</button><button onClick={()=>scrollTo("contact")}>Contact</button>
      </nav>
    ...
    <section className={styles.hero}>
      ...
      <div className={styles.heroCopy}><p>INTERIORS / 2026</p><h1 data-personalize="hero-headline">{STUDIO.headline}</h1><div className={styles.heroBottom}><p data-personalize="hero-intro">{STUDIO.intro}</p><button onClick={()=>scrollTo("tour")}><span>Enter the space</span><ArrowDown/></button></div></div>
    ...
    <section className={styles.tourSection} id="tour" ref={tourRef}>
      <div className={styles.tourSticky}>
        <video className={styles.tourAmbient} ref={ambientVideoRef} src={TOUR_VIDEO} muted playsInline preload="metadata" aria-hidden="true"/><div className={styles.tourVignette}/><div className={styles.tourGrid}/><video className={styles.tourVideo} ref={videoRef} src={TOUR_VIDEO} muted playsInline preload="auto" poster={IMAGES.hero}/>
        <div className={styles.tourTopbar}><span>SCROLL-CONTROLLED TOUR</span><span className={styles.tourReady}>TOUR READY</span></div>
        <div className={styles.tourChapter} key={chapterData.label}><span>{chapterData.label}</span><h2>{chapterData.title}</h2></div>
        <div className={styles.tourRail}><span>SCROLL TO MOVE</span><div className={styles.progressTrack}><div ref={progressRef} className={styles.progressFill}/></div><span>KEEP GOING</span></div>
      </div>
    </section>
