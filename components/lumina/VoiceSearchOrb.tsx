...
function speakThen(text:string,onEnd?:()=>void){
 ...
 message.lang=navigator.language||"en-US";message.rate=1;message.pitch=1;message.volume=1;
 let done=false;const finish=()=>{if(done)return;done=true;onEnd?.()};message.onend=finish;message.onerror=finish;
 window.speechSynthesis.speak(message);window.setTimeout(finish,1800);
}
...
export default function VoiceSearchOrb({left,top}:{left:number;top:number}){
 ...
 useEffect(()=>{let alive=true;const base=supabaseUrl();if(!base)return;fetch(`${base}/functions/v1/voice-speak`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:"What can I do for you?",language:(navigator.language||"en").startsWith("fr")?"fr":"en"})}).then(async response=>{if(!response.ok)throw new Error("tts_failed");const blob=await response.blob();if(!alive)return;const url=URL.createObjectURL(blob);promptUrl.current=url;const audio=new Audio(url);audio.preload="auto";audio.load();promptAudio.current=audio}).catch(()=>{});return()=>{alive=false;if(promptUrl.current)URL.revokeObjectURL(promptUrl.current);promptAudio.current=null;promptUrl.current=""}},[]);
 ...
 function playNaturalPrompt(onEnd:()=>void){
  const audio=promptAudio.current;
  if(!audio){speakThen("What can I do for you?",onEnd);return}
  try{audio.pause();audio.currentTime=0}catch{}
  let done=false;const finish=()=>{if(done)return;done=true;audio.onended=null;audio.onerror=null;onEnd()};
  audio.onended=finish;audio.onerror=()=>speakThen("What can I do for you?",finish);
  const result=audio.play();
  if(result&&typeof result.catch==="function")result.catch(()=>speakThen("What can I do for you?",finish));
  window.setTimeout(finish,2600);
 }
 ...
 async function transcribeRecording(blob:Blob){try{setState("preparing");setCaption("Understanding…");const base=supabaseUrl();if(!base)throw new Error("Voice service unavailable");const mime=blob.type||"audio/mp4";const response=await fetch(`${base}/functions/v1/voice-transcribe`,{method:"POST",headers:{"Content-Type":mime,"X-Audio-Mime":mime,"X-Language":navigator.language||"en-US"},body:blob});const data=await response.json().catch(()=>({}));if(!response.ok||!data?.text)throw new Error(data?.error||`voice_http_${response.status}`);search(String(data.text))}catch(reason){console.error("YNOT voice transcription failed",reason);setState("error");const message=reason instanceof Error?reason.message:"";setCaption(message.includes("deepgram_api_key_not_configured")?"Deepgram key missing in Supabase":message.toLowerCase().includes("invalid")||message.toLowerCase().includes("unauthorized")||message.includes("401")?"Deepgram key rejected — check Supabase secret":"I couldn’t understand that. Tap to retry.");resetLater()}}
 async function recordFallback(skipPrompt=false){try{if(!navigator.mediaDevices?.getUserMedia||typeof MediaRecorder==="undefined")throw new Error("recording_unavailable");setState("asking");setCaption("Allow microphone access…");let media:MediaStream;try{media=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true}})}catch{media=await navigator.mediaDevices.getUserMedia({audio:true})}stream.current=media;const formats=["audio/mp4;codecs=mp4a.40.2","audio/mp4","audio/webm;codecs=opus","audio/webm"],mimeType=formats.find(type=>{try{return MediaRecorder.isTypeSupported(type)}catch{return false}}),rec=new MediaRecorder(media,mimeType?{mimeType}:undefined);recorder.current=rec;const chunks:BlobPart[]=[];rec.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};rec.onerror=()=>{cleanup();recorder.current=null;setState("error");setCaption("Microphone unavailable. Tap to retry.");resetLater()};rec.onstop=()=>{clearTimer();stopTracks();recorder.current=null;const blob=new Blob(chunks,{type:rec.mimeType||mimeType||"audio/mp4"});if(blob.size<300){setState("error");setCaption("I didn’t hear anything. Tap again.");resetLater();return}void transcribeRecording(blob)};const startRecording=()=>{try{if(rec.state!=="inactive")return;rec.start();setState("listening");setCaption("Listening… tap again when done");timer.current=window.setTimeout(()=>{timer.current=null;if(rec.state==="recording")rec.stop()},4800)}catch(reason){console.error("YNOT recorder start failed",reason);cleanup();recorder.current=null;setState("error");setCaption("Microphone could not start. Tap to retry.");resetLater()}};if(skipPrompt){startRecording()}else{setState("asking");setCaption("What can I do for you?");speakThen("What can I do for you?",startRecording)}}catch(reason){console.error("YNOT Safari microphone failed",reason);cleanup();recorder.current=null;setState("error");setCaption("Microphone unavailable. Check Safari permission.");resetLater()}}
