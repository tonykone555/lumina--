...
import {Bell,ChevronDown,Compass,Link2,Search,UserRound} from "lucide-react";
...
export default function AppShell(){const[mode,setMode]=useState<Mode>("shop"),[source,setSource]=useState<Source>("Shopify"),[sourceOpen,setSourceOpen]=useState(false),[bottomQuery,setBottomQuery]=useState(""),[chromeHidden,setChromeHidden]=useState(false),[selectedTags,setSelectedTags]=useState<string[]>([]),[account,setAccount]=useState<Account>(null),[discoverLoading,setDiscoverLoading]=useState(false),[discoverCount,setDiscoverCount]=useState(0),[secondaryReady,setSecondaryReady]=useState(false);const effectiveQuery=useMemo(()=>appendMissingTags(bottomQuery,selectedTags),[bottomQuery,selectedTags]);const sourceMenuRef=useRef<HTMLDivElement>(null);
