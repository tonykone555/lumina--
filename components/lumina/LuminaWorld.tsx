...
function sourceKind(p:Product){const s=(p.source||"").toLowerCase();if(s.includes("ebay"))return"eBay Marketplace";if(s.includes("amazon")||s.includes("shopify"))return"LuminaMarket";if(s.includes("aliexpress"))return"Marketplace";return"Store"}
...
export default function LuminaWorld(){
 ...
 const fetchProducts=useCallback(async(base:string,direction="",append=false,marketOverride:MarketMode=market)=>{
  if(loadingRef.current)return;loadingRef.current=true;append?setLoadingMore(true):setLoading(true);setMarketError("");
  try{const params=new URLSearchParams({q:base||scene.query,market:marketOverride});if(direction)params.set("direction",direction);const r=await fetch(`/api/catalog?${params}`);const data:CatalogPage=await r.json();const incoming=dedupe(data.products||[]);if(data.error)setMarketError(data.error);setProducts(prev=>{if(!append)return layoutProducts(incoming);const merged=dedupe([...prev,...incoming]);return layoutProducts(merged,prev.length)})}catch{setMarketError(marketOverride==="ebay"?"eBay Marketplace is temporarily unavailable":"LuminaMarket is temporarily unavailable")}finally{loadingRef.current=false;setLoading(false);setLoadingMore(false)}
 },[scene.query,market]);
 ...
 const submit=()=>{const q=query.trim();if(!q)return;setSubmitted(q);setFocus("");setSelected(null);setZoom(.9);setPan({x:-WORLD_CX*.9,y:-WORLD_CY*.9});waveRef.current=0;void fetchProducts(q)};
 const explore=(label:string)=>{setFocus(label);setSelected(null);setZoom(z=>Math.max(1.08,z));waveRef.current=0;void fetchProducts(submitted||scene.query,label)};
 const resetWorld=()=>{setCategoryKey("retail");setSubmitted("");setQuery("");setFocus("");setProducts([]);setSelected(null);setMarketError("");setZoom(START_ZOOM);setPan({x:-WORLD_CX*START_ZOOM,y:-WORLD_CY*START_ZOOM})};
 ...
 const changeMarket=(next:MarketMode)=>{if(next===market)return;setMarket(next);setSelected(null);setMarketError("");waveRef.current=0;if(submitted||products.length)void fetchProducts(submitted||scene.query,focus,false,next)};
 ...
 return <main className={`lv4-shell market-${market}`} style={{"--a":scene.a,"--b":scene.b,"--c":scene.c,"--scene":`url(${scene.image})`} as React.CSSProperties}>
  ...
  <header className="lv4-search lv4-search-visible"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder={market==="ebay"?"Search eBay Marketplace — product, style, price…":"Search LuminaMarket — products, style, price…"} aria-label="Search products"/><button onClick={submit}>↵</button></header>
  <div className="lv4-market-toggle" role="group" aria-label="Shopping source"><button className={market==="lumina"?"active":""} onClick={()=>changeMarket("lumina")}><b>LuminaMarket</b><span>Curated stores</span></button><button className={market==="ebay"?"active ebay":"ebay"} onClick={()=>changeMarket("ebay")}><b>eBay</b><span>Marketplace</span></button></div>
  <button className="lv4-mobile-menu" onClick={()=>setMobileNav(v=>!v)}>L@</button>

  <section ref={worldRef} className={`lv4-world level-${level}`} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onWheel={e=>{e.preventDefault();const zoomingIn=e.deltaY<0;setZoom(z=>Math.min(1.75,Math.max(.42,z*(zoomingIn?1.08:.92))));if(zoomingIn&&products.length)loadWave()}}>
   <div className="lv4-stage" style={{width:WORLD_W,height:WORLD_H,transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}>
    {level==="worlds"&&<>
      <div className="lv4-world-title" style={{left:WORLD_CX,top:WORLD_CY-420}}><small>{market==="ebay"?"EXPLORE EBAY":"EXPLORE LUMINA"}</small><strong>Choose a world</strong><span>or search for anything above</span></div>
    ...
    {level!=="worlds"&&<button className="lv4-intent" style={{left:WORLD_CX,top:WORLD_CY}} onClick={()=>{setZoom(z=>Math.min(1.45,z+.18));loadWave()}}><span>{focus||CATEGORIES.find(c=>c.key===categoryKey)?.label}</span><small>{marketError?marketError:loading?"Finding products…":`${products.length} ${market==="ebay"?"eBay":"LuminaMarket"} products${loadingMore?" · more arriving":""}`}</small></button>}
  ...
  {selected&&<aside className="lv4-detail"><button className="lv4-close" onClick={()=>setSelected(null)}><X/></button><img src={selected.image} alt={selected.title}/><div className="lv4-detailcopy"><small className="lv4-product-brand">{selected.brand||"Independent store"}</small><span className={`lv4-product-source ${selected.source?.includes("ebay")?"ebay":""}`}>{sourceKind(selected)}</span><h2>{selected.title}</h2><strong>{money(selected)}</strong><div className="lv4-tagrow">{(selected.tags||[]).slice(0,4).map(t=><button key={t} onClick={()=>explore(t)}>{t}</button>)}</div><div className="lv4-actions"><button className={liked.has(selected.id)?"active":""} onClick={()=>setLiked(s=>{const n=new Set(s);n.has(selected.id)?n.delete(selected.id):n.add(selected.id);return n})}><Heart/></button><a href={selected.url||"#"} target="_blank" rel="noreferrer">{selected.source?.includes("ebay")?"View on eBay":"View product"} <ExternalLink/></a>{categoryKey==="fashion"&&<button className="try"><Sparkles/> Try on</button>}</div></div></aside>}
 </main>
}
