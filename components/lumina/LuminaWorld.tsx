...
function layoutProducts(list:Product[],freshFrom=Number.POSITIVE_INFINITY,splitSources=false){let shopifyIndex=0,amazonIndex=0,otherIndex=0;return list.map((p,i)=>{const source=productSource(p);let local=i,offset=0;if(splitSources){if(source==="shopify"){local=shopifyIndex++;offset=-980}else if(source==="amazon"){local=amazonIndex++;offset=980}else{local=otherIndex++}}return placeProduct(p,local,i>=freshFrom,offset)})}
 ...
 return <main className={`lv4-shell market-${market}`} style={{"--a":scene.a,"--b":scene.b,"--c":scene.c,"--scene":`url(${scene.image})`} as React.CSSProperties}>
  ...
  <section className={`lv4-world level-${level}`} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onWheel={e=>{e.preventDefault();const inward=e.deltaY<0;setZoom(z=>Math.min(2.1,Math.max(.38,z*(inward?1.085:.925))));if(inward)expandWorld()}}>
   <div className="lv4-stage" style={{width:WORLD_W,height:WORLD_H,transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}>
    {level==="worlds"&&<><div className="lv4-world-title" style={{left:WORLD_CX,top:WORLD_CY}}><small>{market==="ebay"?"EXPLORE EBAY":`EXPLORE ${sourceName(luminaSource).toUpperCase()}`}</small><strong>Choose a world</strong><span>or search for anything above</span></div>{CATEGORIES.map((c,i)=>{const a=i/CATEGORIES.length*Math.PI*2-Math.PI/2,x=WORLD_CX+Math.cos(a)*800,y=WORLD_CY+Math.sin(a)*800;return <button key={c.key} className={`lv4-category-bubble cat-${c.key}`} style={{left:x,top:y}} onClick={()=>chooseCategory(c.key)}><b>{c.label}</b><span>{c.subtitle}</span></button>})}</>}
