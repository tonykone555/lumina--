...
export default function LuminaWorld(){
 ...
 useEffect(()=>{if(!submitted||market!=="lumina"||luminaSource!=="shopify"||products.length>=SHOPIFY_WARM_TARGET||loadingRef.current||!shopifyCursorRef.current)return;const t=window.setTimeout(()=>{if(loadingRef.current||!shopifyCursorRef.current)return;const page=waveRef.current+1;waveRef.current=page;void fetchProducts(submitted,shopifyPageDirectionRef.current,true,"lumina",page,"shopify")},140);return()=>window.clearTimeout(t)},[submitted,market,luminaSource,products.length,shopifyFetchSerial,loading,fetchProducts]); // eslint-disable-line react-hooks/exhaustive-deps
