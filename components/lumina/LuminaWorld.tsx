...
type Product={id:string;title:string;brand:string;price:number|null;currency?:string;image:string;url?:string;tags?:string[];source?:string;checkout?:{mode:"ynot"|"merchant";reason:string};x?:number;y?:number;zone?:number;fresh?:boolean};
...
export default function LuminaWorld(){
 ...
 function chooseCategory(key:string){const c=CATEGORIES.find(x=>x.key===key)||CATEGORIES[CATEGORIES.length-1];setCategoryKey(c.key);setSubmitted(c.query);setQuery("");setFocus("");setSelected(null);setHovered(null);setProducts([]);setMarketError("");resetPaging();centerWorld(.93);void fetchProducts(c.query,"",false,market,0,luminaSource)}
 function submit(){const q=query.trim();if(!q)return;setSubmitted(q);setFocus("");setSelected(null);setHovered(null);setCategoryKey("retail");setProducts([]);setMarketError("");resetPaging();centerWorld(.95);void fetchProducts(q,"",false,market,0,luminaSource)}
 ...
 function changeMarket(next:MarketMode){if(next===market)return;setMarket(next);setSourceOpen(false);setSelected(null);setHovered(null);setProducts([]);setMarketError("");resetPaging();if(submitted)void fetchProducts(submitted,focus,false,next,0,luminaSource)}
 function changeLuminaSource(next:LuminaSource){setSourceOpen(false);if(next===luminaSource)return;setLuminaSource(next);setSelected(null);setHovered(null);setProducts([]);setMarketError("");resetPaging();if(submitted)void fetchProducts(submitted,focus,false,"lumina",0,next)}
 function zoomAround(clientX:number,clientY:number,next:number){const stageX=(clientX-pan.x)/zoom,stageY=(clientY-pan.y)/zoom;setZoom(next);setPan({x:clientX-stageX*next,y:clientY-stageY*next})}
 ...
 return <main className={`lv4-shell market-${market}`} style={{"--a":scene.a,"--b":scene.b,"--c":scene.c,"--scene":`url(${scene.image})`} as React.CSSProperties}>
  ...
  <section className={`lv4-world level-${displayLevel}`} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onWheel={wheelZoom}>
   <div className="lv4-stage" style={{width:WORLD_W,height:WORLD_H,transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}>
    ...
    {displayLevel!=="worlds"&&Array.from({length:Math.max(0,zoneCount-1)},(_,i)=>{const zone=i+1,offset=market==="lumina"&&luminaSource==="all"?(zone%2?-980:980):0,previousOffset=market==="lumina"&&luminaSource==="all"?((zone-1)%2?-980:980):0,center=zoneCenter(zone,offset),previous=zoneCenter(zone-1,previousOffset),distance=Math.hypot(center.x-previous.x,center.y-previous.y),angle=Math.atan2(center.y-previous.y,center.x-previous.x),start=zone*PRODUCTS_PER_ZONE+1,end=Math.min(products.length,(zone+1)*PRODUCTS_PER_ZONE);return <Fragment key={`zone-${zone}`}><div className="lv4-zone-connector" style={{left:previous.x,top:previous.y,width:distance,transform:`rotate(${angle}rad)`}}/><div className="lv4-zone-relation" style={{left:(previous.x+center.x)/2,top:(previous.y+center.y)/2}}>{focus||"More discoveries"}</div><div className="lv4-zone-annotation" style={{left:center.x,top:center.y}}><small>{focus||CATEGORIES.find(c=>c.key===categoryKey)?.label||"Discover"}</small><b>Explore further</b><span>{start}–{end} · {market==="lumina"?sourceName(luminaSource):"eBay"}</span></div></Fragment>})}
  ...
  {selected&&<aside className="lv4-detail"><button className="lv4-close" onClick={()=>setSelected(null)}><X/></button><img src={selected.image} alt={cleanTitle(selected.title)}/><div className="lv4-detailcopy"><small className="lv4-product-brand">{selected.brand||"Independent store"}</small><span className={`lv4-product-source ${selected.source?.includes("ebay")?"ebay":""}`}>{sourceKind(selected)}</span><h2>{cleanTitle(selected.title)}</h2><strong>{money(selected)}</strong><div className="lv4-actions">{selected.checkout?.mode==="ynot"?<button disabled={checkoutBusy} onClick={async()=>{setCheckoutBusy(true);setCheckoutError("");try{const q=await fetch("/api/checkout/quote",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...selected,category:categoryKey})});const quote=await q.json();if(!q.ok)throw new Error(quote.error||"Checkout unavailable");const s=await fetch("/api/checkout/session",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:quote.token})});const session=await s.json();if(!s.ok||!session.url)throw new Error(session.error||"Checkout unavailable");window.location.assign(session.url)}catch(e){setCheckoutError(e instanceof Error?e.message:"Checkout unavailable")}finally{setCheckoutBusy(false)}}}><ShoppingBag/> {checkoutBusy?"Checking price…":"Buy with YNOT"}</button>:<a href={selected.url||"#"} target="_blank" rel="noreferrer"><ShoppingBag/> Continue to checkout <ExternalLink/></a>}<button className={liked.has(selected.id)?"active":""} aria-label="Save product" onClick={()=>setLiked(s=>{const n=new Set(s);n.has(selected.id)?n.delete(selected.id):n.add(selected.id);return n})}><Heart/></button>{categoryKey==="fashion"&&<button className="try"><Sparkles/> Try on</button>}</div><small className="lv4-checkout-note">{selected.checkout?.mode==="ynot"?"Price and availability are checked again before secure YNOT payment":`${checkoutCopy(selected)} · payment options and final total are confirmed there`}</small>{checkoutError&&<small className="lv4-checkout-note">{checkoutError}</small>}<div className="lv4-direction-row"><button onClick={()=>branch("More like this")}>More like this</button><button onClick={()=>branch("Cheaper")}>Cheaper</button><button onClick={()=>branch("More premium")}>More premium</button><button onClick={()=>branch("Same shape")}>Same shape</button></div><div className="lv4-tagrow">{(selected.tags||[]).slice(0,5).map(t=><button key={t} onClick={()=>branch(t)}>{t}</button>)}</div></div></aside>}
 </main>
}
