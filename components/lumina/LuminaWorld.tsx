...
type Product={id:string;variantId?:string;title:string;brand:string;price:number|null;currency?:string;image:string;images?:string[];url?:string;tags?:string[];source?:string;variants?:Variant[];checkout?:{mode:"ynot"|"merchant";reason:string};x?:number;y?:number;zone?:number;fresh?:boolean};
...
export default function LuminaWorld(){
 ...
 useEffect(()=>{try{const saved=JSON.parse(localStorage.getItem("ynot-saved-products")||"[]");if(Array.isArray(saved))setLiked(new Set(saved.map(String)))}catch{}},[]);
 ...
 function toggleSaved(id:string){setLiked(current=>{const next=new Set(current);next.has(id)?next.delete(id):next.add(id);try{localStorage.setItem("ynot-saved-products",JSON.stringify([...next]))}catch{}return next})}
