...
function placeProduct(p:Product,index:number,fresh=false,offsetX=0){
 ...
 const rawX=center.x+Math.cos(angle)*ring,rawY=center.y+Math.sin(angle)*ring,safe=keepClearOfBigBubbles(rawX,rawY,hash(p.id||p.title)+index);
 return{...p,title:cleanTitle(p.title),zone,fresh,x:safe.x,y:safe.y,z:[-.3,0,.3][local%3]};
}
...
export default function LuminaWorld(){
 ...
 const fetchProducts=useCallback(async(base:string,direction="",append=false,marketOverride:MarketMode=market,page=0,sourceOverride:LuminaSource=luminaSource,promoteIncoming=false)=>{
  ...
  try{
   const params=new URLSearchParams({q:base||scene.query,market:marketOverride,page:String(Math.max(0,page)),country:shopperCountry()});
 ...
 function chooseCategory(key:string){const c=CATEGORIES.find(x=>x.key===key)||CATEGORIES[CATEGORIES.length-1];setCategoryKey(c.key);setSubmitted(c.query);setQuery("");setFocus("");setSelected(null);setSuggestionsOpen(false);setHovered(null);setSortMode("discovery");setColourFilter("");setShapeFilter("");setProducts([]);setMarketError("");resetPaging();centerWorld(.93);void fetchProducts(c.query,"",false,market,0,luminaSource)}
 function submit(){const q=query.trim();if(!q)return;setSubmitted(q);setFocus("");setSelected(null);setSuggestionsOpen(false);setHovered(null);setSortMode("discovery");setColourFilter("");setShapeFilter("");setCategoryKey("retail");setProducts([]);setMarketError("");resetPaging();centerWorld(.95);void fetchProducts(q,"",false,market,0,luminaSource)}
 ...
 function resetWorld(){replaceRequestRef.current++;setSubmitted("");setQuery("");setFocus("");setSelected(null);setSuggestionsOpen(false);setHovered(null);setProducts([]);setMarketError("");resetPaging();loadingRef.current=false;centerWorld(START_ZOOM)}
 ...
 useEffect(()=>{setVisualSimilar([]);setSuggestionsOpen(false)},[selected?.id]);
 ...
 return <main className={`lv4-shell market-${market} scene-${categoryKey} depth-${displayLevel}`} style={{"--a":scene.a,"--b":scene.b,"--c":scene.c,"--scene":`url(${background||scene.image})`} as React.CSSProperties}>
  ...
  {selected&&typeof document!=="undefined"&&createPortal(<button type="button" className="lv4-mobile-floating-close" aria-label="Close product" onPointerDown={event=>{event.preventDefault();event.stopPropagation();setSelected(null);setSuggestionsOpen(false);setCheckoutError("")}} onTouchStart={event=>{event.preventDefault();event.stopPropagation();setSelected(null);setSuggestionsOpen(false);setCheckoutError("")}} onClick={event=>{event.preventDefault();event.stopPropagation();setSelected(null);setSuggestionsOpen(false);setCheckoutError("")}}><X/></button>,document.body)}{selected&&<aside className="lv4-detail"><button className="lv4-close" type="button" onPointerDown={event=>{event.preventDefault();event.stopPropagation();setSelected(null);setSuggestionsOpen(false);setCheckoutError("")}} onTouchStart={event=>{event.preventDefault();event.stopPropagation();setSelected(null);setSuggestionsOpen(false);setCheckoutError("")}} onClick={event=>{event.preventDefault();event.stopPropagation();setSelected(null);setSuggestionsOpen(false);setCheckoutError("")}} aria-label="Close product"><X/></button><img src={selected.image} alt={cleanTitle(selected.title)}/>{(selected.images||[]).length>1&&<div className="lv4-gallery" aria-label="Product photos">{selected.images!.map((image,index)=><button key={image} className={selected.image===image?"active":""} onClick={()=>setSelected(product=>product?{...product,image}:product)}><img src={image} alt={`${cleanTitle(selected.title)} view ${index+1}`}/></button>)}</div>}<div className="lv4-detailcopy"><small className="lv4-product-brand">{selected.brand||"Independent store"}</small><span className={`lv4-product-source ${selected.source?.includes("ebay")?"ebay":""}`}>{sourceKind(selected)}</span><h2>{cleanTitle(selected.title)}</h2><strong>{money(selected)}</strong>{selectableVariants(selected).length>0&&<div className="lv4-variants" aria-label="Product options">{selectableVariants(selected).map(v=><button key={v.id} disabled={!v.available} className={selected.variantId===v.id?"active":""} onClick={()=>setSelected(p=>p?{...p,variantId:v.id,price:v.price,currency:v.currency||p.currency,image:v.image||p.image,url:v.url||p.url}:p)}>{v.label}{v.price!=null?` · ${money({...selected,price:v.price,currency:v.currency||selected.currency})}`:""}</button>)}</div>}<div className="lv4-actions"><button disabled={checkoutBusy}><ShoppingBag/>YNOT BAG</button><button className={`lv4-save-action ${liked.has(selected.id)?"active":""}`} aria-label={liked.has(selected.id)?"Remove from saves":"Save product"} onClick={()=>toggleSaved(selected)}><Heart/></button>{categoryKey==="fashion"&&<button className="try"><Sparkles/> Try on</button>}</div>{checkoutError&&<div className="lv4-checkout-fallback"><small>{checkoutError}. You have not been charged.</small>{similar.length>0&&<div>{similar.map(p=><button key={p.id} onClick={()=>{setSelected(p);setCheckoutError("")}}><img src={p.image} alt=""/><span>{cleanTitle(p.title)}</span></button>)}</div>}</div>}<div className="lv4-similar-block"><button type="button" className={"lv4-similar-pill "+(suggestionsOpen?"active":"")} onClick={()=>{const next=!suggestionsOpen;setSuggestionsOpen(next);if(next)void loadVisualSimilar()}}><Sparkles/>{suggestionsOpen?"Hide similar":visualSimilarLoading?"Finding visual matches…":"Similar picks"}</button>{suggestionsOpen&&<div className="lv4-similar-slider" aria-label="Visually similar products and brands">{displayedSimilar.map(p=><button type="button" key={p.id} onClick={()=>{setSelected(p);setSuggestionsOpen(false)}}><img src={p.image} alt=""/><span><b>{p.brand||"Independent store"}</b><small>{cleanTitle(p.title)}</small><em>{money(p)}</em></span></button>)}</div>}</div><div className="lv4-direction-row"><button onClick={()=>branch("More like this")}>More like this</button><button onClick={()=>branch("Cheaper")}>Cheaper</button><button onClick={()=>branch("More premium")}>More premium</button><button onClick={()=>branch("Same shape")}>Same shape</button></div></div></aside>}
 </main>
}
