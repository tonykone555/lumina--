...
export default function LuminaWorld(){
 ...
 useEffect(()=>{const change=(event:Event)=>{const far=Boolean((event as CustomEvent<{far?:boolean}>).detail?.far);if(!far){centerWorld(submitted?.78:START_ZOOM);return}let minX=Infinity,maxX=-Infinity,minY=Infinity,maxY=-Infinity,count=0;for(const product of products){const x=Number(product.x),y=Number(product.y);if(!Number.isFinite(x)||!Number.isFinite(y)||Math.abs(x)>WORLD_W*2||Math.abs(y)>WORLD_H*2)continue;minX=Math.min(minX,x);maxX=Math.max(maxX,x);minY=Math.min(minY,y);maxY=Math.max(maxY,y);count+=1}if(!count){centerWorld(.34);return}const padding=260,width=Math.max(1,maxX-minX+padding*2),height=Math.max(1,maxY-minY+padding*2),viewportW=Math.max(320,window.innerWidth-100),viewportH=Math.max(320,window.innerHeight-130),fit=Math.min(viewportW/width,viewportH/height,.5),next=Number.isFinite(fit)?Math.max(MIN_ZOOM,fit):MIN_ZOOM,centerX=(minX+maxX)/2,centerY=(minY+maxY)/2;if(!Number.isFinite(centerX)||!Number.isFinite(centerY))return;window.requestAnimationFrame(()=>{setZoom(next);setPan({x:-centerX*next,y:-centerY*next})})};window.addEventListener("ynot:set-world-overview",change as EventListener);return()=>window.removeEventListener("ynot:set-world-overview",change as EventListener)},[products,submitted]);
 ...
 function zoomAround(clientX:number,clientY:number,next:number){const safe=Math.max(MIN_ZOOM,next),stageX=(clientX-pan.x)/zoom,stageY=(clientY-pan.y)/zoom;setZoom(safe);setPan({x:clientX-stageX*safe,y:clientY-stageY*safe})}
