...
export default function LuminaWorld(){
 ...
 useEffect(()=>{const change=(event:Event)=>{const far=Boolean((event as CustomEvent<{far?:boolean}>).detail?.far);if(!far){centerWorld(submitted?.78:START_ZOOM);return}const visible=products.filter(p=>Number.isFinite(p.x)&&Number.isFinite(p.y));if(!visible.length){centerWorld(.34);return}const padding=260,minX=Math.min(...visible.map(p=>Number(p.x)))-padding,maxX=Math.max(...visible.map(p=>Number(p.x)))+padding,minY=Math.min(...visible.map(p=>Number(p.y)))-padding,maxY=Math.max(...visible.map(p=>Number(p.y)))+padding;const fit=Math.min((window.innerWidth-100)/(maxX-minX),(window.innerHeight-130)/(maxY-minY),.5);const next=Math.max(MIN_ZOOM,fit);setZoom(next);setPan({x:-((minX+maxX)/2)*next,y:-((minY+maxY)/2)*next})};window.addEventListener("ynot:set-world-overview",change as EventListener);return()=>window.removeEventListener("ynot:set-world-overview",change as EventListener)},[products,submitted]);
