...
export default function LuminaWorld(){
 ...
 const zoneDetails=useMemo(()=>Array.from({length:Math.max(1,Math.ceil(products.length/PRODUCTS_PER_ZONE))},(_,zone)=>{const counts=new Map<string,number>();products.slice(zone*PRODUCTS_PER_ZONE,(zone+1)*PRODUCTS_PER_ZONE).flatMap(productDetails).forEach(tag=>counts.set(tag,(counts.get(tag)||0)+1));return[...counts].sort((a,b)=>b[1]-a[1]).slice(0,3).map(([tag])=>tag)}),[products]);
