...
export default function WorldCompass():React.ReactElement{
 ...
 useEffect(()=>{
  const toggle=()=>{setFar(false);setOpen(value=>!value)};
  const onHeaderCompass=(event:Event)=>{
   const target=event.target as Element|null;
   const button=target?.closest<HTMLButtonElement>(".ynot-reference-nav-tools > button");
   if(!button)return;
   event.preventDefault();
   event.stopPropagation();
   toggle();
  };
  window.addEventListener("ynot:toggle-compass",toggle);
  document.addEventListener("click",onHeaderCompass,true);
  return()=>{window.removeEventListener("ynot:toggle-compass",toggle);document.removeEventListener("click",onHeaderCompass,true)};
 },[]);
