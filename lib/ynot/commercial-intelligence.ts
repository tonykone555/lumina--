...
export function productQuality(p:CatalogueProduct){const imgs=[p.image,...(p.images||[])].filter(Boolean);let score=Math.min(35,imgs.length*7);if(String(p.image||"").startsWith("https://"))score+=15;if(String(p.title||"").length>14)score+=12;if((p.tags||[]).length>2)score+=12;if(Number(p.price)>0)score+=12;if(p.brand)score+=8;if(p.url&&p.url!=="#")score+=6;return clamp(score)}
...
export async function optionalAiJson<T>(system:string,input:any,fallback:T):Promise<{usedAi:boolean,value:T}> {const key=String(process.env.OPENAI_API_KEY||"");if(!key)return {usedAi:false,value:fallback};try{const model=String(process.env.YNOT_INTELLIGENCE_MODEL||"gpt-5.6-mini");const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({model,input:[{role:"system",content:[{type:"input_text",text:system}]},{role:"user",content:[{type:"input_text",text:JSON.stringify(input)}]}],text:{format:{type:"json_object"}}})});if(!response.ok)return {usedAi:false,value:fallback};const data=await response.json();const text=data?.output?.flatMap((o:any)=>o?.content||[]).find((c:any)=>c?.type==="output_text")?.text;if(!text)return {usedAi:false,value:fallback};return {usedAi:true,value:JSON.parse(text) as T}}catch{return {usedAi:false,value:fallback}}}
