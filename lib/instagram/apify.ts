...
const FALLBACK_KEYWORD_ACTOR=process.env.APIFY_FALLBACK_KEYWORD_ACTOR||"maximedupre~instagram-user-search-scraper";
...
function matchedKeyword(row:Record<string,unknown>,fallback:string){const list=row.matchedKeywords;if(Array.isArray(list)&&list.length)return s(list[0])||fallback;return firstString(row.query,row.search_query,row.sourceQuery,fallback)}
...
function parseKeywordRows(rows:Record<string,unknown>[],fallbackQuery:string){
 const profiles:InstagramProfile[]=[];const seen=new Set<string>();
 for(const row of rows){const profile=profileFromRow(row,"keyword",fallbackQuery);if(!profile||seen.has(profile.username))continue;seen.add(profile.username);profiles.push(profile)}
 return profiles;
}
...
export async function keywordSearch(queries:string[],maxPagesPerQuery=10){
 ...
 let profiles=parseKeywordRows(rows,cleanQueries[0]);
 if(profiles.length)return profiles;
 const maxItems=Math.max(20,Math.min(200,cleanQueries.length*Math.max(1,Math.min(10,maxPagesPerQuery))*20));
 const fallbackRows=await runActor(FALLBACK_KEYWORD_ACTOR,{keywords:cleanQueries,maxItems});
 profiles=parseKeywordRows(fallbackRows,cleanQueries[0]);
...
export async function relatedSearch(seeds:string[],maxResultsPerProfile=80){
 ...
 for(const row of rows){
  const profile=profileFromRow(row,"related");
  const parent=normalizedUsername(row.seed_username||row.seedUsername||row.seed_profile||row.seedProfile||row.seed);
  if(!profile||!parent||seen.has(`${parent}:${profile.username}`))continue;
  seen.add(`${parent}:${profile.username}`);profiles.push(profile);
  edges.push({parentUsername:parent,childUsername:profile.username,edgeType:"instagram_suggested",rank:profile.rank});
 }
