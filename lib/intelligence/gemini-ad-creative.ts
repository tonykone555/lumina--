...
const MODEL=/^gemini-(?:2\.5)-flash$/i.test(REQUESTED_MODEL)?"gemini-3.6-flash":REQUESTED_MODEL;
...
export async function analyseAdForProduct(input:{ad:SourceAd;product:Product;contextAds?:SourceAd[];sourceAnalysis?:any}){
 ...
 const prompt=`You are YNOT's paid-social creative strategist. The complete source ad was already analysed as MARKET RESEARCH. Adapt only its useful underlying marketing principles to OUR PRODUCT. Never copy a competitor logo, exact wording, distinctive trade dress, branded character, or near-identical composition.\n\nSOURCE AD\n${JSON.stringify({platform:ad.platform,advertiser:ad.advertiser,title:ad.title,headline:ad.headline,body:ad.body,cta:ad.cta,format:ad.format,started:ad.started,landingPage:ad.landingPage},null,2)}\n\nSOURCE ANALYSIS\n${JSON.stringify(input.sourceAnalysis||{},null,2)}\n\nOUR PRODUCT\n${JSON.stringify({id:product.product_id||product.id,title:product.title,brand:product.brand,description:product.description,price:product.price,currency:product.currency,advertability_score:product.advertability_score},null,2)}\n\nRELATED ADS\n${JSON.stringify(context.map(x=>({advertiser:x.advertiser,headline:x.headline,body:x.body,cta:x.cta,format:x.format})),null,2)}\n\nReturn ONLY JSON:\n{\n "analysis":{"niche":"","sub_niche":"","audience":{"segment":"","intent":"","price_sensitivity":""},"creative_type":"","format":"","style":{"visual_style":"","lighting":"","composition":"","background":"","color_palette":[],"text_overlay_style":""},"marketing_structure":{"hook":"","problem":"","promise":"","offer":"","cta":""},"visual_elements":[],"what_to_keep":[],"what_to_change":[],"risk_flags":[]},\n "adaptation":{"reason_for_match":"","adapted_angle":"","adapted_benefit":"","price_positioning":"","cta_direction":"","tone":"","constraints":[]},\n "directions":[{"direction_id":"dir_01","name":"","goal":"","hook":"","overlay_text":[],"layout_concept":"","prompt":"","negative_prompt":[],"format":"4:5","variant_count":4,"scene_type":"product|human|ugc|editorial|problem-solution","human_use":false,"product_interaction":"","variation_axis":""}]\n}\nCreate EXACTLY 5 directions and make them visually and strategically DIFFERENT, not five rewrites of the same shot. Use these five roles:\n1) PRODUCT HERO: no human, premium clean product-first composition.\n2) HUMAN USE / DEMONSTRATION: MUST include a photorealistic adult human using, wearing, holding, applying, sitting on, operating, or otherwise interacting with the product in the physically correct way for that exact product category. Set human_use=true and scene_type="human". The person must not merely stand beside the product.\n3) UGC / CANDID: phone-native, believable creator/customer perspective, materially different framing and environment.\n4) PROBLEM / SOLUTION: visually communicate the consumer problem and product benefit with a different scene structure.\n5) EDITORIAL / OFFER: a distinct campaign/editorial or offer-led composition with different camera angle, lighting, props and layout.\n\nEvery direction must differ in at least environment, camera distance/angle, subject placement, lighting, prop set and emotional intent. Do not reuse the same background or composition across directions. Prompts must explicitly preserve the supplied YNOT product's exact identity, silhouette, materials, colors, packaging, logo/label and proportions. The product reference is an object to place into a NEW scene, not an image canvas to repaint.`;
 ...
 if(out.directions.length>=2&&!out.directions.some((d:any)=>d?.human_use===true||String(d?.scene_type||"").toLowerCase()==="human")){
  out.directions[1]={...out.directions[1],scene_type:"human",human_use:true,product_interaction:String(out.directions[1]?.product_interaction||"An adult customer naturally uses the product in the physically correct way."),prompt:`Photorealistic adult customer naturally using the exact referenced product in the physically correct way for this product category. The product must be clearly visible and retain its exact identity, shape, colors, packaging and label. ${String(out.directions[1]?.prompt||"")}`};
 }
